...
async function getRepliesByForumId(forum_id) {
    ...
    const [replies] = await conn.query(
        'SELECT replies.id, users.photo, CONCAT(users.fname, " ", users.lname) as fullname, replies.content, replies.created_at, replies.updated_at FROM replies INNER JOIN users WHERE replies.user_id = users.id AND replies.forum_id = ? ORDER BY replies.created_at DESC',
        [forum_id]
    );
